...
from app.database import (
    ...
    get_active_public_nodes, get_all_public_nodes, get_public_nodes_stats
)
...
app = FastAPI(title="Milijon Proxy Worker", version="1.1.0")
...
async def get_system_stats(request: Request):
    ...
    harvester_stats = get_public_nodes_stats()
...
# --- PUBLIC NODE HARVESTER ENDPOINTS ---

@app.get("/api/harvester/nodes")
async def get_nodes_endpoint(request: Request, limit: int = 50):
    check_auth(request)
    return get_all_public_nodes(limit=limit)

@app.get("/api/harvester/stats")
async def get_harvester_stats_endpoint(request: Request):
    check_auth(request)
    return get_public_nodes_stats()

@app.post("/api/harvester/refresh")
async def trigger_harvester_refresh(request: Request):
    check_auth(request)
    result = await harvest_and_check_nodes()
